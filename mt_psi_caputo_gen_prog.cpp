...
//////////////////////// genetic programming //////////////////////////////////////////
// constants
double max_val=1e300; // maximal fitness function value
double max_c=10; // maximal constant node value
double log_base_min=1.5; // minimal value of logarithm base
int t_max_depth=2; // maximal generated tree depth
int max_nodes=100; // maximal number of nodes in a tree
int pop_size=100; // population size
double mut_prob=0.05; // probability of mutation
int max_iters=5000; // maximal number of iterations
double eps1=0.1;
double eps2=1e-10;
int debug=1;
int silent=0;
int cub_ints=1;
/////////////////////////////////////////////////////////////////////////////////
////////////// genetic programming - function description as a tree /////////////
/////////////////////////////////////////////////////////////////////////////////
class node;
node *deserialize(char *name,FILE *fi=NULL);
class node
{
friend node *deserialize(char *name,FILE *fi);
    int type; // 0 - a+b, 1 - a*b, 2 - a^b, 3 - ln_{log_base_min+c}(1+a), 4 - c, 5 - x
    node *a,*b;
    double c;
public:
    node() {a=b=NULL;type=4;c=0.0;}
    void clear() 
    {
		if (type<4) if (a) delete a;
		if (type<3) if (b) delete b; 
		a=b=NULL;
    }
    ~node() { clear(); }
    double val(double x,int *con=NULL,int opt=0) // calculates tree value
    {
		double v=0.0;
		int c1=1,c2=1;
		if (con) con[0]=0;
		...
		if (type==5) v=x;
		if (type==0) v=a->val(x,&c1,opt)+b->val(x,&c2,opt);
		if (type==1) v=a->val(x,&c1,opt)*b->val(x,&c2,opt);
		if (type==2) v=pow(a->val(x,&c1,opt),b->val(x,&c2,opt));
		if (type==3) v=log(1.0+a->val(x,&c1,opt))/log(log_base_min+c);
		if (!finite(v)) v=max_val;
		if (opt)
		{
		    if (type<=3) // simplify if arguments are constants
		    if (c1 && c2)
		    {
			c=v;
			delete a;
		    ...
		    if ((type==0)||(type==1)) // simplify + and * constant chains
		    {
    ...
    {
	if (type==4) // root cannot be a number - change to x
	{
	    printf("opt451 ");
	    type=5;
	    return;
	...
	val(0.0,NULL,1);
	if (type==4) // root cannot be a number - change to x - check twice after another optimizations
	{
	    printf("opt452 ");
	    type=5;
	    return;
    ...
    void gen_subtree(int max_depth,int depth=0) // build random tree with depth up to max_depth
    {
		if (max_depth<0) max_depth=0;
		do
			type=rand()%6;
		while ((max_depth==depth)&&(type!=4)&&(type!=5));
		if (type==4) 
			c=max_c*((rand()%1000000)/1000000.0);
		if (type<=3)
		{
			a=new node(max_depth,depth+1);
			if (type!=3)
			b=new node(max_depth,depth+1);
			else
			c=max_c*((rand()%1000000)/1000000.0);
		}
    }
    node(int max_depth,int depth=0)
    {
		gen_subtree(max_depth,depth);
    }
    void mutate(int max_depth) // change subtree to a randomly generated subtree with given maximal depth
    {
		clear();
		gen_subtree(max_depth);
    }
    ...
    void print()
    {
		...
		if (type==3) { printf("log_{%g}(1.0+(",log_base_min+c);a->print();printf("))"); }
    }
...
node *deserialize(char *name,FILE *fi)
{
	node *n=new node();
	FILE *f=fi;
	if (fi==NULL)
	...
	if (n->type<=3)
	{
		n->a=deserialize(name,f);
		if (n->type!=3)
		    n->b=deserialize(name,f);
	}
	if (fi==NULL)
	    fclose(f);
	return n;
...
int func_in_kernel = 0; // 0 -x, 1 - x^1/2, 2 - x^2, 3 - x^k, 4 - sigmoid, 5 - tree based function description
int integr_max_niter=100000;
int recursive_max_n=10000;
double recursive_max_v=1000;
...
double g(double t)
{
	...
	if (func_in_kernel==5) // tree based description
		return f4->val(t)-f4->val(0);
	return t;
}
...
public:
	...
	double _kb(int n, double t0, double t1, double gtj,int sing,double alpha)
	{
		...
		if ((!finite(v1))||(!finite(v2)))
		    return max_val;
		if ((fabs(v1)>recursive_max_v)||(fabs(v2)>recursive_max_v))
		    return max_val;
		if (n>recursive_max_n)
		    return v2;
		if ((t1 - t0) < global_eps) 
		{
			if (sing)
...
omp_unset_lock(&lock);
	...
	double calc_int(double alpha, int t0,int t1,int a,double dL,int time)
	{
		...
		if ((func_in_kernel==0)&&(cub_ints==0)) // analytic form for g(x)=x
		    return _kb_row(t0*dL,t1*dL,g(a*dL), alpha);
		if ((func_in_kernel==5)||(cub_ints==1)) // only cubature for tree based g(x) description
			return _kb(1, t0*dL,t1*dL,g(a*dL),t1==a,alpha);			
		// series expansion methods in other cases
...
#pragma omp parallel for		
			...
			for (int tt0=0;tt0<aa;tt0++)
			{
			    ...
			    if (silent==0)
				if ((aa==100)||(aa==200)||(aa==300))
				    printf("space_ints %d %d %g\n",aa,tt0,kb_space[aa*(sN+2)+tt0]);
			}
		}
		double sfp;
		int ff4=0;
		node *sf4;
		if (func_in_kernel==5) {sf4=f4;f4=f4t;}
		if (func_in_kernel==4) {func_in_kernel=3;ff4=1;}
		if (func_in_kernel==3)
		{
		    sfp=func_power;
...
#pragma omp parallel for
		for (int aa=0;aa<=tstep;aa++)
		{
		    ...
		    if (silent==0)
		    if (tstep==200)
			printf("time_ints %d %d %g\n",tstep,aa,kb_time[aa]);
		}
		if (func_in_kernel==5) f4=sf4;
		if (func_in_kernel==3) func_power=sfp;		
		if (ff4==1) func_in_kernel=4;
		//////
		if (isEV)
		    set_EV_TR();
		U1();
...
///////////////////////////////////////////////////////////////////////////////////////
double fit_and_solve(double t,double save_tau,double out_tau,int irr,int bc,double tau_m,
	double *init_Z, double *init_F, int ninit,
	double *perc_T, double *perc_A, int nperc,
	double *irr_T, double *irr_A, int nirr,
...
{
	node **population; // trees for space derivative
	node **population_t; // trees for time derivative
	double *ffs;
	int iter;
	// other params
	double t,save_tau,out_tau;
	int irr,bc;
	double tau_m;
	...
	double ev_mu;
	int nEV_T,nEV_F, to_fit, pso_n;
	double pso_o, pso_fi_p, pso_fi_g, pso_eps;
	int pso_max_iter;
	int fit;
	double *param_values;
	int nparam_values;
	char *et_file,*pv_file;
public:
	double fitness_function(node *n,node *nt)
	{
		func_in_kernel=5;
		if (debug)
		{
		     printf("calculating fitness function for:\n");
		     n->print();
		     printf("\n");
		     nt->print();
		     printf("\n");
		...
		int t1=GetTickCount();
		double v=fit_and_solve(t,save_tau,out_tau, irr,bc, tau_m, init_Z, init_F, ninit, perc_T, perc_A,
	                         nperc, irr_T, irr_A, nirr, values_T, values_Z, values_F, nvalues, check_T, check_Z, check_F,
	                         ncheck, EV_T, EV_F, LAI, ev_mu, nEV_T,nEV_F, to_fit, pso_n, pso_o, pso_fi_p, pso_fi_g, pso_eps,
	                         pso_max_iter, 2, param_values, nparam_values, et_file,pv_file);
	        if (debug)
	    	    printf("FF=%g time %d\n",v,GetTickCount()-t1);
	    	return v;
	}
	genetic_programming(double _t,double _save_tau,double _out_tau,int _irr,int _bc,double _tau_m,
	double *_init_Z, double *_init_F, int _ninit,
	double *_perc_T, double *_perc_A, int _nperc,
	double *_irr_T, double *_irr_A, int _nirr,
	double *_values_T,double *_values_Z,double *_values_F,int _nvalues,
	double *_check_T,double *_check_Z,double *_check_F,int _ncheck,
	double *_EV_T,double **_EV_F,double *_LAI,double _ev_mu,int _nEV_T,int _nEV_F,
	int _to_fit,
	int _pso_n,double _pso_o,double _pso_fi_p,double _pso_fi_g,double _pso_eps,int _pso_max_iter,
	int _fit,double *_param_values,int _nparam_values,char *_et_file,char *_pv_file)
	{
	    iter=0;
		// save parameters
		t=_t;save_tau=_save_tau;out_tau=_out_tau;
		irr=_irr;bc=_bc;
		tau_m=_tau_m;
		init_Z=_init_Z;init_F=_init_F;
		ninit=_ninit;
		perc_T=_perc_T; perc_A=_perc_A;
		nperc=_nperc;
		irr_T=_irr_T;irr_A=_irr_A;
		nirr=_nirr;
		values_T=_values_T;values_Z=_values_Z;values_F=_values_F;
		nvalues=_nvalues;
		check_T=_check_T;check_Z=_check_Z;check_F=_check_F;
		ncheck=_ncheck;
		EV_T=_EV_T;EV_F=_EV_F;LAI=_LAI;
		ev_mu=_ev_mu;
		nEV_T=_nEV_T;nEV_F=_nEV_F; to_fit=_to_fit; pso_n=_pso_n;
		pso_o=_pso_o; pso_fi_p=_pso_fi_p; pso_fi_g=_pso_fi_g; pso_eps=_pso_eps;
		pso_max_iter=_pso_max_iter;
		fit=_fit;
		param_values=_param_values;
		nparam_values=_nparam_values;
		et_file=_et_file;pv_file=_pv_file;
		
	    population=new node*[pop_size];
	    population_t=new node*[pop_size];
	    ffs=new double[pop_size];
	    for (int i=0;i<pop_size;i++)
	    {
			population[i]=new node();
			population_t[i]=new node();
			do
			{
				population[i]->gen_subtree(t_max_depth);
				population_t[i]->gen_subtree(t_max_depth);
				ffs[i]=fitness_function(population[i],population_t[i]);
				if ((!finite(ffs[i]))||(ffs[i]>max_val))
				{
					population[i]->clear();
					population_t[i]->clear();
				}
				else
					break;
			}
	...
	void do_iteration()
	{
		...
		int r1=rand()%s1;
		int r2=rand()%s2;
		int s1_t=population_t[n1]->n_nodes();
		int s2_t=population_t[n2]->n_nodes();
		int r1_t=rand()%s1_t;
		int r2_t=rand()%s2_t;
		// replace a subtree from node 1 with a subtree from node 2
		node *nn1=population[n1]->copy();
		node *nn1_t=population_t[n1]->copy();
		double fnn1;
		nn1->get_node(r1)->replace(population[n2]->get_node(r2)->copy());
		nn1_t->get_node(r1_t)->replace(population_t[n2]->get_node(r2_t)->copy());
		// recalculate fitness function
		fnn1=fitness_function(nn1,nn1_t);
		if (!finite(fnn1)) fnn1=max_val;
		if (fnn1>max_val) fnn1=max_val;
		if (debug) printf("c(%d[%d] %d[%d]) ",n1,r1,n2,r2);
		if (fnn1<=max) // replace maximal ff valued node
		{
			delete population[imax];
			delete population_t[imax];
			population[imax]=nn1;
			population_t[imax]=nn1_t;
			ffs[imax]=fnn1;
		}
		else
		{
	    ...
	    if (r<mut_prob)
	    {
			// select random node and random subtree
			n1=rand()%pop_size;
			n2=rand()%population[n1]->n_nodes();
			int n2_t=rand()%population_t[n1]->n_nodes();
			...
			population_t[n1]->get_node(n2_t)->mutate(t_max_depth);
			// recalculate fitness function
			ffs[n1]=fitness_function(population[n1],population_t[n1]);
			if (!finite(ffs[n1])) ffs[n1]=max_val;
			if (ffs[n1]>max_val) ffs[n1]=max_val;
			if (debug) printf("m(%d[%d]) ",n1,n2);
			if ((ffs[n1]==max_val)||(population[n1]->n_nodes()>max_nodes)||(population_t[n1]->n_nodes()>max_nodes))
			{
				delete population[n1];
				population[n1]=nn1;
				delete population_t[n1];
				population_t[n1]=nn1_t;
				ffs[n1]=sf1;
				if (debug) printf("c2 ");
			}
			else
	...
	void optimize(double &minval,node *&res, node *&res_t)
	{
	    res=NULL;
	    res_t=NULL;
	    ...
	    do
	    {
			double max=0,min=max_val,avg=0;
			do_iteration();
			for (int i=0;i<pop_size;i++)
			{
				if (ffs[i]<min) {min=ffs[i];res=population[i];res_t=population_t[i];}
			...
			{
				if (debug) printf ("%d->(%g,%g)\n",iter,min,max);
				oldmin=min;
				oldmax=max;
				res->serialize("f4.txt");
				res_t->serialize("f4t.txt");
			}
			if (debug)
			    printf("genprog_iter %d->(%g,%g,%g)\n",iter,min,avg,max);
...
void gen_prog(double t,double save_tau,double out_tau,int irr,int bc,double tau_m,
	double *init_Z, double *init_F, int ninit,
	double *perc_T, double *perc_A, int nperc,
	double *irr_T, double *irr_A, int nirr,
	double *values_T,double *values_Z,double *values_F,int nvalues,
	...
	int fit,double *param_values,int nparam_values,char *et_file,char *pv_file)
{
    ...
    double min;
	silent=1;
	genetic_programming *g=new genetic_programming(t,save_tau,out_tau,irr,bc,tau_m,
                                                    init_Z, init_F, ninit, perc_T, perc_A, nperc, irr_T, irr_A, nirr,
                                                    values_T, values_Z, values_F, nvalues, check_T, check_Z, check_F, ncheck,
                                                    EV_T, EV_F, LAI, ev_mu, nEV_T, nEV_F, to_fit, pso_n, pso_o, pso_fi_p, pso_fi_g, pso_eps, pso_max_iter,
                                                    fit, param_values, nparam_values, et_file, pv_file);
    
	g->optimize(min,best,best_t);
	silent=0;
	g->fitness_function(best,best_t);
	printf("genetic programming best value - %g\n",min);
	if ((best)&&(best_t))
    {
        printf("Function parameter of space-fractional derivative:\n");
        best->print();
        printf("\n");
        printf("Function parameter of time-fractional derivative:\n");
        best_t->print();
        printf("\n");
    }
	delete g;
...
int main(int argc,char **argv)
{
	...
	for (int i=1;i<argc;i+=2)
	{
		...
		if (strstr(argv[i],"vgm")!=NULL)
		{
		    ...
		    if (fi)
		    {
                ...
                vgm_nlayers=0;
                fseek(fi,0,SEEK_SET);			
                while (fscanf(fi,"%lg %lg %lg %lg %lg %lg %lg %lg\n",vgm_s0s+vgm_nlayers,vgm_s1s+vgm_nlayers,vgm_ns+vgm_nlayers,vgm_as+vgm_nlayers,
                                                                     vgm_h0+vgm_nlayers,vgm_h1+vgm_nlayers,vgm_k+vgm_nlayers,av_ps+vgm_nlayers)==8)
                {
		...
		if (strcmp(argv[i], "newrand") == 0)
			srand(time(NULL));
		// gen prog parameters
		if (strcmp(argv[i], "gp_max_val") == 0)
			max_val = atof(argv[i + 1]);
		if (strcmp(argv[i], "gp_max_c") == 0)
			max_c = atof(argv[i + 1]);
		if (strcmp(argv[i], "gp_log_base_min") == 0)
			log_base_min = atof(argv[i + 1]);
		if (strcmp(argv[i], "gp_t_max_depth") == 0)
			t_max_depth = atoi(argv[i + 1]);
		if (strcmp(argv[i], "gp_max_nodes") == 0)
			max_nodes= atoi(argv[i + 1]);
		if (strcmp(argv[i], "gp_pop_size") == 0)
			pop_size = atoi(argv[i + 1]);
		if (strcmp(argv[i], "gp_mut_prob") == 0)
			mut_prob = atof(argv[i + 1]);
		if (strcmp(argv[i], "gp_max_iters") == 0)
			max_iters = atof(argv[i + 1]);
		if (strcmp(argv[i], "gp_eps1") == 0)
			eps1 = atof(argv[i + 1]);
		if (strcmp(argv[i], "gp_eps2") == 0)
			eps2 = atof(argv[i + 1]);
		if (strcmp(argv[i], "gp_debug") == 0)
			debug = atoi(argv[i + 1]);
		if (strcmp(argv[i], "gp_silent") == 0)
			silent = atoi(argv[i + 1]);
		if (strcmp(argv[i], "cub_ints") == 0)
			cub_ints = atoi(argv[i + 1]);
		if (strcmp(argv[i], "gp_f4") == 0)
			f4 = deserialize(argv[i + 1]);
		if (strcmp(argv[i], "gp_f4t") == 0)
			f4t = deserialize(argv[i + 1]);
	}
	sN=N=(BS*NB);
	sNB = NB;
	...
	if (fit!=0)
	{
		...
		// run
		if (fit==3)
            gen_prog(Tm,Sm,Om,irr,bc,tau_m,init_Z,init_F,ninit,perc_T,perc_A,nperc,irr_T,irr_A,
                     nirr,values_T,values_Z,values_F,nvalues,check_T,check_Z,check_F,ncheck,
                     EV_T,EV_F,LAI,ev_mu,nEV_T,nEV_F,to_fit,pso_n,pso_o,pso_fi_p,pso_fi_g,pso_eps,
                     pso_max_iter,fit,&param_values[0],nparam_values,et_file,pv_file);
        else		
            fit_and_solve(Tm,Sm,Om,irr,bc,tau_m,init_Z,init_F,ninit,perc_T,perc_A,nperc,irr_T,irr_A,
                          nirr,values_T,values_Z,values_F,nvalues,check_T,check_Z,check_F,ncheck,
                          EV_T,EV_F,LAI,ev_mu,nEV_T,nEV_F,to_fit,pso_n,pso_o,pso_fi_p,pso_fi_g,pso_eps,
                          pso_max_iter,fit,&param_values[0],nparam_values,et_file,pv_file);
	}
